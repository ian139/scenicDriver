...
def compute_scenic_score(
    ...
    """
    Compute an aggregate scenic score from multiple classification results.

    This is useful for scoring a route or region from multiple tile classifications.

    Args:
        classification_results: List of classification dicts from batch_classify
        aggregation: How to aggregate scores:
            - "mean": Simple average of scenic weights
            - "weighted_mean": Average weighted by confidence
            - "max": Maximum scenic weight (most scenic spot)
            - "percentile_90": 90th percentile scenic weight

    Returns:
        Aggregate scenic score (0-1)
    """
    if not classification_results:
        return 0.0

    scenic_weights = [r["scenic_weight"] for r in classification_results]
    confidences = [r["confidence"] for r in classification_results]

    if aggregation == "mean":
        return np.mean(scenic_weights)
    elif aggregation == "weighted_mean":
        total_conf = sum(confidences)
        if total_conf == 0:
            return np.mean(scenic_weights)
        return sum(w * c for w, c in zip(scenic_weights, confidences)) / total_conf
    elif aggregation == "max":
        return max(scenic_weights)
    elif aggregation == "percentile_90":
        return np.percentile(scenic_weights, 90)
    else:
...
def get_class_distribution(
    ...
    Returns:
        List of (class_name, count, percentage) tuples, sorted by count
    """
    ...
    class_counts = Counter(r["class"] for r in classification_results)
    total = len(classification_results)

    return [
        (cls, count, count / total)
        for cls, count in class_counts.most_common(top_n)
    ]
