...
from src.scenic_scorer.active_evaluation import promote_from_decision, rollback_registry


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Promote or rollback a regression model registry")
    p.add_argument("--decision", type=Path)
    p.add_argument("--candidate-checkpoint", type=Path)
    p.add_argument("--registry", "--registry-json", dest="registry", type=Path,
    ...
    if args.expected_registry_sha256 is None:
        p.error("--expected-registry-sha256 is required (use the current registry SHA256)")
    return args
...
def main() -> int:
    args = parse_args()
    if args.rollback:
        result = rollback_registry(args.registry, args.target_history_index, args.expected_registry_sha256)
    else:
        result = promote_from_decision(
            args.decision, args.candidate_checkpoint, args.registry,
            args.expected_registry_sha256, args.run_name,
        )
