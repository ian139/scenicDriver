...
- [ ] Revalidate the fixed 2,256-case workload after the max-detour policy
  correction. The published Vast runs predate the intentional change to use
  the unrestricted fastest route as the detour reference for all 1,128
  `avoid_highways=true` cases, so they remain historical scalar-bound evidence
  rather than current-policy performance validation. See the policy boundary
  in `docs/research/routing-performance-autoresearch.md`.
