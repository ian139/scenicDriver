...
- [x] Unified endpoint-aware routing — integrated at `6a103d21` (routing implementation at `1d990b7b`), superseding `dc820d58`.
  ...
  Scenic frontier initialization reuses the exact fastest endpoint request and
  remains deadline-, cancellation-, and mutation-safe. Collapsed real-node
...
- [x] Corrected full 2,256-case Vast benchmark from `49b10789` — r15 completed and published on offer `40429508` (256 effective CPU cores, 1,031,851 MiB RAM, 2 workers, group size 64). It persisted all 2,256 canonical cases and passed every completed-route correctness invariant, including `q0_fastest: pass=1563`, baseline presence, duration-cap, highway, objective, route-metric, certification, and diagnostic consistency. Results: 1,563 completed, 497 timeouts, 196 no-route results; fixed-denominator median/p95 `6,418.755/10,915.547 ms`, and 1,758 cases (77.93%) under 10 seconds. Compared with corrected r8, under-10 completion rose 16.98 percentage points and p95 improved 6.82%, but fixed-denominator median worsened 7.85%; this is not a complete latency-SLA gate.
- [x] Comparable r16 completed on offer `45436164` (Xeon E5-2686 v4, 72 effective CPU cores, 125.7 GiB RAM, 2 workers, group size 64), then the host was destroyed. All 2,256 cases persisted and the final/checkpoint objects were published under `outputs/vast/new-england-north-full-bbox-v1-r16-comparable-e5/`; checkpoint fingerprint `ebeb021e5a986338313d0724c4ac4c8b0eb0ed6abd4201138f26239754d7b750`, final SHA-256 `a22b78ed3199ced363d6fcba2835f87bd0befbbaee8858504363b42030ee8f27`, checkpoint SHA-256 `bf534670c31b8c7702bc01dd2539b3e66c45ea0a670882b05c2c72a0366d8d05`. All completed-route invariants passed (`q0_fastest: pass=1408`); 1,408 completed, 652 timeouts, 196 no-route results; fixed-denominator median/p95 `9,347.644/12,521.992 ms`, and 1,603 cases (71.05%) under 10 seconds. Relative to corrected r8, under-10 completion improved 10.10 points, but fixed-denominator median and p95 worsened; the latency gate remains open.
- [x] Fixed-denominator latency gate resolved by the bounded scenic scalar traversal retained at `86bbfde3` and merged at `ba5cf73b`: on the same fixed 2,256-case EPYC workload, all-case median/p95 improved from `6,449.169/12,274.275 ms` to `1,238.558/4,473.816 ms`, with 2,254 cases (99.91%) under 10 seconds. All 1,568 commonly completed cases retained identical route snapshots and non-timing behavior; the candidate added 490 completions and lost none. See `docs/research/routing-performance-autoresearch.md` for the complete experiment record and durable S3 artifacts.
- [ ] Revalidate the fixed 2,256-case workload after the route-control policy
  corrections. Highway avoidance is now best effort: strict highway-free
  routing falls back under the same unrestricted-fastest cap and request
  deadline with a highway-exposure penalty. The published Vast runs predate
  both this correction and the unrestricted detour-reference correction for
  all 1,128 `avoid_highways=true` cases, so they remain historical scalar-bound
  evidence rather than current-policy performance validation. Local
  correctness verification passed 533 Python tests and 14 viewer tests; a live
  Augusta→Lewiston request changed from HTTP 422 to a valid fallback route with
...
7. Keep future routing optimization evidence-driven: profile the retained
   scalar-bound implementation before reconsidering frontier pruning, CCH,
   or MLD, and require fixed-denominator Vast validation for retention.
